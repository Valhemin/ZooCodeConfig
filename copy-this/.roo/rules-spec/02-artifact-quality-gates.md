...
Run `spec-consistency-analysis` before handing off. **Only required for new projects or large features with a full spec pack.** For small requests (single endpoint, minor feature), a quick self-check suffices — run `spec-consistency-analysis` only if something feels inconsistent.
